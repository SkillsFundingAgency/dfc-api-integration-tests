...
namespace DFC.Api.JobProfiles.IntegrationTests.Test
{
    public class DetailTest : SetUpAndTearDown
    {
        ...
        {
            Response<JobDetails> authorisedAPIResponseWithContent = await this.CommonAction.ExecuteGetRequest<JobDetails>(Settings.APIConfig.EndpointBaseUrl.ProfileDetail + this.CanonicalName).ConfigureAwait(true);
        ...
        {
            Response<JobDetails> authorisedAPIResponseNoContent = await this.CommonAction.ExecuteGetRequest<JobDetails>(Settings.APIConfig.EndpointBaseUrl.ProfileDetail + this.CommonAction.RandomString(10).ToLower(CultureInfo.CurrentCulture)).ConfigureAwait(true);
        ...
        {
            Response<JobDetails> unauthorisedAPIResponse = await this.CommonAction.ExecuteGetRequest<JobDetails>(Settings.APIConfig.EndpointBaseUrl.ProfileDetail + this.CanonicalName, false).ConfigureAwait(true);
