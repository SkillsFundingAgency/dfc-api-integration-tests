...
namespace DFC.Api.JobProfiles.IntegrationTests.Test.Create
{
    public class SummaryTest : SetUpAndTearDown
    {
        ...
        {
            Response<List<JobSummary>> authorisedAPIResponseWithContent = await CommonAction.ExecuteGetRequest<List<JobSummary>>(Settings.APIConfig.EndpointBaseUrl.ProfileSummary).ConfigureAwait(true);
        ...
        {
            Response<List<JobSummary>> unauthorisedAPIResponse = await CommonAction.ExecuteGetRequest<List<JobSummary>>(Settings.APIConfig.EndpointBaseUrl.ProfileSummary, false).ConfigureAwait(true);
